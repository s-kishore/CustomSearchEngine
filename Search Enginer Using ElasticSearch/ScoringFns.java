...
public class ScoringFns {
	...
	static long TOT_DOC_COUNT = esConstants.TOT_DOC_COUNT;
	static double OBM25_CONS_K1 = esConstants.OBM25_K1;
	static double OBM25_CONS_K2 = esConstants.OBM25_K2;
	static double OBM25_CONS_B = esConstants.OBM25_B;
	static double LAMBDA = esConstants.LAMBDA;
	static long VOC_SIZE = esConstants.VOC_SIZE;
	static long RESULT_SIZE = esConstants.NUMBER_OF_RESULTS;
			...
			Map<String, Integer> qarray2, long corpusLength1) 
	{
		...
		avg_doc_legth = (double)corpusLength/esConstants.TOT_DOC_COUNT;
		beginScoring();
		writeResults();
	}
	...
	private static void writeResults() 
	{
		...
		try 
		{
			PrintWriter wrtr_TF = new PrintWriter(esConstants.RESULT_OKAPI_TF);
			PrintWriter wrtr_IDF = new PrintWriter(esConstants.RESULT_OKAPI_IDF);
			PrintWriter wrtr_BM25 = new PrintWriter(esConstants.RESULT_OKAPI_BM25);
			PrintWriter wrtr_L = new PrintWriter(esConstants.RESULT_LAPLACE);
			PrintWriter wrtr_JM= new PrintWriter(esConstants.RESULT_JM);
	...
	private static double calcJelinekMercer(double tfd, double cur_doc_length,String term) 
	{
		double jm_part1 = LAMBDA * (tfd/cur_doc_length);
		double jm_part2 = (1- LAMBDA) * ((double)word_ttf.get(term)/(double)corpusLength);
		return Math.log(jm_part1 + jm_part2);
	}
	...
	private static double calcOkapiTF(double tfd, double dfw, double cur_doc_length) 
	{
